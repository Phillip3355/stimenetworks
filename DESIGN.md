...
- 서버 소개문 아래에는 `image copy 6`을 넓은 대표 장면으로 배치하고, `image copy 7·8·9`를 이어지는 보조 장면으로 전시한다. 기간이 끝난 런칭 카운트다운과 별도의 하단 오픈 전시는 사용하지 않는다.
...
- 서버 소개 쇼케이스는 데스크톱에서 대표 장면을 21:9로 넓게 보여주고, 보조 장면 세 개를 같은 너비의 3열로 이어 붙인다.
...
- 서버 소개 쇼케이스는 모바일에서 `image copy 6→7→8→9` 순서의 한 열로 쌓고, 430px 이하에서는 각 장면을 4:3으로 전환한다.
